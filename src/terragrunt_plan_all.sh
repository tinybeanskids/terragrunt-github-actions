...
function terragruntPlanAll {
  ...
  planOutput=$(${tfBinary} plan-all -detailed-exitcode -input=false -lock=false ${*} 2>&1 | grep -v "\[terragrunt]" | grep -v -e "Refreshing state..." -e "Refreshing Terraform" -e "refreshed state")
  planExitCode=${?}
